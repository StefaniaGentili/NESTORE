function [nclus,nclusA,nclusB]=feacalc_nostop_ti(filein, fileout,interv, label, PL, Zth, start_time, end_time,yearchMc, Mc1,Mc2)
...
filein=fullfile('../data/Clusters/',filein);
...
fileout=fullfile('../data/Training_Output/',fileout);
fid=fopen(fileout,'w');
...
for(k=1:length(main))
    ...
    if label==0 ...
        ...
        if(~isempty(ind_s))
            ...
            if(round(dm*10)>=8 & round(dm*10)<=12)
                ind_s=[];
            end
        ...
        if(~isempty(ind_s))
            ...
            timeN=find(JD_ld<=s2b & JD_ld>=s1 & MD_ld>=mainmag-2);
